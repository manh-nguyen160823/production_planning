odoo.define('erpvn_planning_management.DemandVariantMixin', function (require) {
'use strict';
...
var DemandVariantMixin = {
    ...
    _getCombinationInfo: function (ev) {
        ...
        var productTemplateId = parseInt($parent.find('.product_tmpl_id').val());

        self._checkExclusions($parent, combination);

        return ajax.jsonRpc(this._getUri('/erpvn_planning_management/get_combination_info_demand'), 'call', {
    ...
    onChangeAddQuantity: function (ev) {
        ...
        if ($(ev.currentTarget).closest('.oe_demand_optional_products_modal').length > 0){
            $parent = $(ev.currentTarget).closest('.oe_demand_optional_products_modal');
    ...
    selectOrCreateProduct: function ($container, productId, productTemplateId, useAjax) {
        ...
        if (productId) {
            ...
            var route = '/erpvn_planning_management/create_product_variant';
            if (useAjax) {
