odoo.define('erpvn_planning_management.ProductConfiguratorFormControl', function (require) {
"use strict";

var core = require('web.core');
var _t = core._t;
var FormController = require('web.FormController');
var OptionalProductsModalDemand = require('erpvn_planning_management.OptionalProductsModalDemand');

var ProductDemandConfiguratorFormControl = FormController.extend({
    custom_events_demand: _.extend({}, FormController.prototype.custom_events_demand, {
        field_changed: '_onFieldChanged_change_product',
    ...
    willStart: function () {
        ...
        if (this.initialState.data.product_tmpl_id) {
            return this._configureProduct_demand(
                this.initialState.data.product_tmpl_id.data.id
            ).then(function () {
                return def;
            });
        }
    ...
    */
    _onButtonClicked: function (event) {
        ...
        if (attrs.special === 'cancel') {
            ...
            if (!this.$el
                    .parents('.modal')
                    .find('.o_erpvn_planning_management_add')
                    .hasClass('disabled')) {
                this._handleAdd();
            }
        }
     ...
     * @private
     */
    _onFieldChanged_change_product: function (event) {
        this._super.apply(this, arguments);

        var self = this;
        var productId = event.data.changes.product_tmpl_id.id;
        ...
        this._configureProduct_demand(event.data.changes.product_tmpl_id.id)
            .then(function () {
     ...
     */
    _configureProduct_demand: function (productTemplateId) {
        var self = this;
        var initialProduct = this.initialState.data.product_tmpl_id;
        ...
        return this._rpc({
            route: '/erpvn_planning_management/configure_product_demand',
            params: {
    ...
    */
    _handleAdd: function () {
        ...
        var productTemplateId = $modal.find('.product_tmpl_id').val();
        this.renderer.selectOrCreateProduct(
            ...
            self.OptionalProductsModalDemand = new OptionalProductsModalDemand($('body'), {
            ...
            self.OptionalProductsModalDemand.on('options_empty', null,
                // no optional products found for this product, only add the root product
                self._onAddRootProductOnly.bind(self));

            self.OptionalProductsModalDemand.on('update_quantity', null,
                self._onOptionsUpdateQuantity.bind(self));

            self.OptionalProductsModalDemand.on('confirm', null,
                self._onModalConfirm.bind(self));

            self.OptionalProductsModalDemand.on('closed', null,
    ...
    _onModalConfirm: function () {
        ...
        this._addProducts(this.OptionalProductsModalDemand.getSelectedProducts());
    },
...
return ProductDemandConfiguratorFormControl;
