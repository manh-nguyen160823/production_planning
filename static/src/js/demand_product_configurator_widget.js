odoo.define('erpvn_planning_management.origin_product_configurator', function (require) {
...
/**
 * The erpvn_planning_management.product_configurator widget is a simple widget extending FieldMany2One
 ...
 */
var DemandProductConfiguratorWidget = relationalFields.FieldMany2One.extend({
    ...
    /**
     * Add button linking to product_id/product_tmpl_id form.
     */
    ...
    /**
     * Override catching changes on product_id or product_tmpl_id.
     ...
     *     from looping forever when it needs to change the 'product_tmpl_id'
     *
     * @private
     */
    reset: async function (record, ev) {
        ...
        if (ev && ev.target === this) {
            if (ev.data.changes && !ev.data.preventProductIdCheck && ev.data.changes.product_tmpl_id) {
     ...
     * (options, ...).
     * Allows demand_product_configurator module to apply its options
...
FieldsRegistry.add('product_demand_configurator', DemandProductConfiguratorWidget);
